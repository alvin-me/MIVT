...
vec3 calcGradient(sampler3D volume, VolumeParameters volumeStruct, vec3 samplePos) {
  ...
  float v0 = textureLookup3D(volume, volumeStruct, samplePos + vec3(offset.x, 0.0, 0.0)).a;
  float v1 = textureLookup3D(volume, volumeStruct, samplePos + vec3(0, offset.y, 0)).a;
  float v2 = textureLookup3D(volume, volumeStruct, samplePos + vec3(0, 0, offset.z)).a;
  float v3 = textureLookup3D(volume, volumeStruct, samplePos + vec3(-offset.x, 0, 0)).a;
  float v4 = textureLookup3D(volume, volumeStruct, samplePos + vec3(0, -offset.y, 0)).a;
  float v5 = textureLookup3D(volume, volumeStruct, samplePos + vec3(0, 0, -offset.z)).a;
